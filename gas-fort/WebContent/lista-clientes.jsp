...
<script type="text/javascript">
	...
	function deletar(id) { 			
		...
		$("#dialog-confirm").dialog({
			...
			buttons: {
				"Deletar": function() {
					document.location.href="/gas-fort/deletarCliente.do?id=" + id;
					$(this).dialog("close");
				},
				Cancelar: function() {
					$(this).dialog("close");
...
<c:import url="cabecalho.jsp"></c:import>
	...
	<div id="dialog-confirm" title="Deletar Cliente">
		<p>
			<span style="float: left; margin: 0 20px 20px 0;"><img
				src="imgs/error.png" /></span> Tem certeza que deseja remover esse Cliente?
		</p>
	</div>
	...
	<div id="dialog-confirm-2" title="Alerta" style="font: 12px verdana;">
		<p>
			<span style="float: left; margin: 0 20px 20px 0;">
				<img src="imgs/error.png" /></span>
		</p>
			<div>Esse cadastro não pode ser<br/> excluído!</div>
		<p></p>				
	</div>
	...
	<div id="conteudo">
		...
		<div id="busca">
			<html:form action="/buscaCliente" styleClass="noEnterSubmit"
				focus="valor">
				...
				<input type="button" class="botao-adicionar" value="Adicionar"
					onclick="document.location.href='abreAdicionaCliente.do'" />
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:forEach var="clientes" items="${clientes}">
							<tr>
								<td colspan="2">${clientes.id}</td>
								<td colspan="2">${clientes.nomeFantasia}</td>
								<td colspan="2">${clientes.razaoSocial}</td>

								<td colspan="2" align="center">
								<a href="abreEditaCliente.do?id=${clientes.id}">
								<img src="imgs/edit.png" title="Editar" border="0" /></a> 
								<a href="#" onclick="javascript:deletar(${clientes.id});">
