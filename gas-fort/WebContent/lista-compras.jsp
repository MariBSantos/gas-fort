...
function deletar(id) { 			
	...
	$("#dialog-confirm").dialog({
		...
		buttons: {
			"Deletar": function() {
				document.location.href="/gas-fort/deletarCompra.do?id=" + id;
				$(this).dialog("close");
			},
			Cancelar: function() {
				$(this).dialog("close");
			...
			function anterior(pg){
				...
				if (indice == 1){
					...
					document.location.href='/gas-fort/buscaCompra.do?indice=' + indice + "&valor=" + valor + "&pg=" + pg;
				}
...
<c:import url="cabecalho.jsp"></c:import>
	...
	<div id="dialog-confirm" title="Deletar compra?">
		<p>
			<span style="float: left; margin: 0 20px 20px 0;"><img
				src="imgs/error.png" /></span> Tem certeza que deseja remover essa Compra?
		</p>
	</div>
	...
	<div id="conteudo">
		...
		<div id="titulo">Compras</div>

		<div id="busca">
			<html:form action="/buscaCompra" styleClass="noEnterSubmit" focus="valor">
				...
				<div id="valor-busca">
					...
					<html:text styleId="valor" property="valor" styleClass="caixa-texto-busca" value="${valor}" ></html:text>
				</div>
				...
				<div id="combo-select">
					<div class="label-busca">Valor</div>
						<html:select styleId="fornecedor" property="fornecedor" styleClass="combo-box-busca" value="${valor}">
				...
				<input type="button" class="botao-adicionar" value="Adicionar" onclick="document.location.href='abreCadastroCompra.do'" />
				<html:errors property="codigo"></html:errors>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:forEach var="compras" items="${compras}">
							<tr>
								<td>${compras.id}</td>
								<td>${compras.fornecedor.nomeFantasia}</td>
								<td><fmt:formatDate value="${compras.data}" type="date" pattern="dd/MM/yyyy" /></td>

								<td colspan="2" align="center">
									<a href="abreEditaCompra.do?id=${compras.id}"> 
									<img src="imgs/edit.png" title="Editar" border="0" /></a> 
									<a href="#" onclick="javascript:deletar(${compras.id});"> 
